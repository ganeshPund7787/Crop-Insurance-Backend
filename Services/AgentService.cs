...
public class AgentService : IAgentService
{
    ...
    private async Task<AgentProfile> GetVerifiedAgentAsync(Guid userId)
    {
        ...
        // ── Clearer error messages for each failure case ───────────────────
        if (user.Role != UserRole.InsuranceAgent)
            throw new InvalidOperationException(
                $"User role is '{user.Role}' — not an InsuranceAgent.");

        if (user.AgentProfile is null)
            throw new KeyNotFoundException(
                $"AgentProfile record missing for user {userId}. " +
                "Re-register as InsuranceAgent with agentCode and licenseNumber.");

        if (!user.AgentProfile.IsVerified)
            throw new InvalidOperationException(
                "Agent account is not verified yet. Contact admin.");
